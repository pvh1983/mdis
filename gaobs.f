...
c  individual, and write output information.
         ...
         genavg(i)=fbar
         genmax(i)=best
! hpham28: Print the best guy to best.dat
         Open (unit=333,file="best.dat")
         write(333,*) best  
         close(333)
! hpham28: END		 
...
c
      ...
      rewind 24
      OPEN (UNIT=23, FILE='gaobs.inp', STATUS='OLD')
      READ (23, NML = ga)
      CLOSE (23)
      itourny=1
...
!         call func(j,funcval)
         ...
         Open (unit=333,file="parentobs.dat")
!         do jj=1,npopsiz
!         write(333,*) (parent(ii,jj),ii=1,nparam)
		 write (333,fmt='(<nparam>F8.2)')(parent(ii,j),ii=1,nparam)
!        enddo         
         close(333)

!!!! HPham Calculate fitness values (call Matlab codes -> print 'fitness.dat')
         call system("octave -qH --no-window-system expdsg_final.m")

!          call system("matlab -nodisplay < expdsg_final.m")
!         call system("octave -qH --no-window-system call_myfunc.m") 
!!!!      
         Open (unit=333,file="fitness.dat")
!         do jj=1,npopsiz
         read(333,*) fitness(j)
!        enddo         
         close(333)    
