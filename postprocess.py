...
2. To RUN:
    ...
    - TYPE: postprocess.py (no need 'python postprocess.py')
INPUT: all_pmpx.csv
OUTPUT: figures
...
n_new_pmp_wells = 2  # Choose 1, 2, or 3
nobs = 4  # Max number of new obs wells choose 1 to 4.
wdir = '/work/ftsai/'
...
for s in dsg_sce:
    ...
    for i in range(nobs):
        ...
        ax2.hist(abs(y), bins=50, density=False, histtype='step',
                 label='nobs='+str(i+1))  # color='blue', ls='dashed',
        ax2.set_ylabel('Frequency')
    ax2.set_title(s)
    ax2.legend()  # Add a legend.
    ofile = odir + '/' + 'IG_hist_' + s + '.png'
    fig2.savefig(ofile, dpi=150, transparent=False, bbox_inches='tight')
    print(f'Saved {ofile}')
    plt.close(fig2)
